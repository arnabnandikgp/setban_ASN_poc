...
class DisconnectBanTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("setban: succesfully handling the case of overlapping banperiod") #the banperiod taken here are arbitary only for the pupose of demo 
        self.nodes[1].clearbanned()
        self.nodes[1].setban("12345678", command="add",bantime=10)
        assert_equal(len(self.nodes[1].getpeerinfo()), 0)
        self.nodes[1].setban(subnet="127.0.0.1", command="add",bantime=20)
        assert_equal(len(self.nodes[1].listbanned()), 2)
        self.nodes[1].setmocktime(old_time + 11)
        assert_equal(len(self.nodes[1].listbanned()), 1)
        assert_equal(len(self.nodes[1].listbanned()), 1)
        assert_equal(len(self.nodes[1].getpeerinfo()), 0)
        self.nodes[1].setmocktime(old_time + 21)
